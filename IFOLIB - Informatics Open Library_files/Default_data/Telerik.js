...
Type.registerNamespace("Telerik.Web.UI");
(function(c,a,b){var x=a.proxy,i=".",H=true,G=true,w=false,p=false,y="rsbSearchContext",A="rsbSCInner",z="rsbSCSlide",t="rsbInner",C="rsbEmptyMessage",D="rsbFocused",B="rsbDisabled",o="rsbSlide",n="rsbPopup",j="rsbList",k="rsbListItem",l="rsbListItemHovered",m="rsbListItemTemplate",f="rsbButtons",g="rsbButtonsLeft",h="rsbButtonsRight",d="rsbButton",e="rsbButtonSearch",v="rsbLabel",E="rsbShowAll",u="rsbInput",s="rsbHeader",r="rsbFooter",F="span",q={0:"contains",1:"startswith",2:"eq"};
...
c.RadSearchBox=function(I){c.RadSearchBox.initializeBase(this,[I]);
this._enabled=true;
...
c.RadSearchBox._getTarget=function(I){var J=$telerik.getTouchTarget(I);
if(!a(J).hasClass(k)){return a(J).parents(i+k).get(0);
}return J;
};
c.RadSearchBox._extractErrorMessage=function(I){if(I.get_message){return I.get_message();
}else{return I.replace(/(\d*\|.*)/,"");
}};
c.RadSearchBox._createChildListElement=function(){var I=a("<ul class='"+j+"'></ul>"),K=a(this.get_dropDownElement()).find(i+n),J=K.find(i+r);
if(J.length==1){J.before(I);
}else{I.appendTo(K);
}return this.get_childListElement();
};
c.RadSearchBox._toggleAttribute=function(L,J,K,M){var I=a(L);
if(M){I.attr(J,K);
}else{I.removeAttr(J);
}};
c.RadSearchBox._toggleHoveredState=function(L,J,K){if(this.get_enabled()){var I=a(this._getInnerWrapElement());
if(!I.hasClass(J)){I.toggleClass(K,L);
}}};
c.RadSearchBox._toggleFocusedState=function(L,J,K){if(this.get_enabled()){var I=a(this._getInnerWrapElement());
I.removeClass(K);
I.toggleClass(J,L);
}};
...
}},_initializeEvents:function(){var K=this,I=a(this.get_element()),J=a(this.get_inputElement());
I.on("click",i+d,function(L){if(!K.get_enabled()){return;
}if(a(L.currentTarget).hasClass(e)){K._onSearchButtonClick(L);
}else{K._onButtonClick(L);
}});
J.on("keydown",function(L){K._onKeyDown(L);
}).on("blur",function(L){if(!K._preventBlur){K._onBlur(L);
}K._preventBlur=false;
}).on("focus",function(L){K._onFocus(L);
});
if($telerik.isIE||$telerik.isChrome){J.on("keypress",function(L){if(L.keyCode==Sys.UI.Key.enter){L.preventDefault();
}});
}},_initializeAutoCompleteEvents:function(){var K=this,J=a(this.get_inputElement()),I=a(this.get_dropDownElement());
a(document).on("mousedown."+this._uniqueId,function(L){K._onDocumentMouseDown(L);
});
J.on("mousedown",function(L){L.stopPropagation();
}).on("keyup",function(L){K._onKeyUp(L);
}).on("paste",function(L){K._onPaste(L);
});
I.on("mousedown",function(L){L.stopPropagation();
K._preventBlur=true;
}).on("mouseover",i+k,function(L){K._onItemMouseOver(L);
}).on("mouseout",i+k,function(L){K._onItemMouseOut(L);
}).on("click",i+k,function(L){K._onItemClick(L);
}).on("click",i+E,function(L){K._onShowAllClick(L);
});
},_initializeODataSourceBinder:function(){var J=this.get_odataClientSettings().ODataSourceID,I=$find(J),K;
if(!I){K=String.format("DataSource with id {0} was not found on the page",J);
alert(K);
}else{this._flatBinder=new c.RadODataDataSource.Binder.Flat(I,this);
this._flatBinder.initialize();
}},_initializeWebServiceLoader:function(){var I=this.get_webServiceSettings();
this._webServiceLoader=new c.WebServiceLoader(I);
...
},_createButtonCollection:function(){var J,I,L=0,K=0,M,N=null;
buttonData=this._buttonData;
this._buttons=new c.SearchBoxButtonCollection(this);
if(!buttonData){return;
}J=a(F+i+g,this.get_element()).find(i+d);
I=a(F+i+h,this.get_element()).find(i+d);
for(var O=0,P=buttonData.length;
O<P;
O++){M=new c.SearchBoxButton();
this._buttons.add(M);
if(buttonData[O].position==c.SearchBoxButtonPosition.Left){N=J[L];
L++;
}else{N=I[K];
K++;
}M._initialize(buttonData[O],N);
}},_createSearchContext:function(){if(!this._searchContextData){return;
}var L=this,I=a(this._element),K=I.find(i+y).get(0),J=I.find(i+z).get(0);
this._searchContext=new c.SearchContext(K,J);
a(this._searchContext).on({selectedIndexChanged:function(){L.updateClientState();
},repaint:function(){L._adjustInputWidth();
}});
this._searchContext._initialize(this);
},_buttonPropertyChanged:function(I,J,K){if(this._enableClientStatePersistence){this._log.logPropertyChanged(I,J,K);
}},_logInserted:function(I){if(!I.get_parent()._childControlsCreated||!this._enableClientStatePersistence){return;
}this._log.logInsert(I);
},_logRemoving:function(I){if(this._enableClientStatePersistence){this._log.logDelete(I);
}},_openDropDown:function(){var J=this._dropDown,I=a(this._element).find(i+y).outerWidth();
if(J&&!J.isVisible()){J.show(this.get_element(),null,I);
this._adjustChildListElementHeight();
if(!this._isDropDownWidthSet&&this.get_searchContext()){this._adjustPopupWidth(I);
}}},_closeDropDown:function(){if(this._dropDown){this._dropDown.hide();
}},_adjustChildListElementHeight:function(){var I=a(this.get_dropDownElement()).find(i+n);
if(I[0].style.height===""){return;
}var L=I.height(),K=I.find(i+s).outerHeight(true),J=I.find(i+r).outerHeight(true),M=I.find(i+E).outerHeight(true),N;
if(L>0){N=L-(K+J+M);
a(this.get_childListElement()).height(N);
}},_createChildListItemElement:function(L,I,J){var K=a("<li class='"+I+"'>"+L+"</li>").get(0);
K._data=J;
return K;
},_createDropDownElement:function(){return a("<div class='"+o+"'><div class='"+n+"'></div></div>").get(0);
},_addButtonsWrapper:function(J){var I=this._createButtonsWrapper(J),K=this.get_inputElement();
if(J==g){I.insertBefore(K);
}else{I.insertAfter(K);
}return I.get(0);
},_createButtonsWrapper:function(I){return a("<span class='"+f+" "+I+"'></span>");
},_findButtonsWrapper:function(I){return a(this.get_element()).find(F+i+I).get(0);
},_childInserted:function(I){var K=this._createButtonDomElement(I),J=this._ensureButtonsWrapper(I);
I.set_element(K);
this._appendButtonDomElement(K,J);
},_ensureButtonsWrapper:function(I){var K="",J;
K=(I.get_position()==c.SearchBoxButtonPosition.Left)?g:h;
J=this._findButtonsWrapper(K);
if(!J){J=this._addButtonsWrapper(K);
}return J;
},_childRemoved:function(I){this._destroyButtonDomElement(I.get_element());
I._dispose();
},_createButtonDomElement:function(I){return a(I._render()).get(0);
},_appendButtonDomElement:function(M,L){var K=a(L).find(i+d),O=M._button.get_index(),I=a(M),J;
for(var N=0,P=K.length;
N<P;
N++){J=K[N];
if(J._button.get_index()>O){I.insertBefore(J);
return;
}}I.appendTo(L);
},_destroyButtonDomElement:function(J){var I=a(J),K=J.parentNode;
I.remove();
if(!K.hasChildNodes()){a(K).remove();
}},_populateDropDown:function(I,K){var J=this.get_childListElement()||c.RadSearchBox._createChildListElement.call(this),L=I.length;
if(L>0){J.innerHTML="";
I.appendTo(J);
if(K&&L>=this.get_maxResultCount()){this._addShowAllButton();
}else{this._removeShowAllButton();
}this._openDropDown();
if(this._highlightFirstMatch){this._keyboardNavigator._highlightItem(I[0]);
}}else{this.clear();
}},_addShowAllButton:function(){var I=a(this.get_childListElement());
if(I.siblings(i+E).length==0){I.after("<div class='"+E+"'><span>"+this._showAllResultsText+"</span></div>");
this._adjustChildListElementHeight();
}},_removeShowAllButton:function(){var I=a(this.get_childListElement()).siblings(i+E);
if(I.length>0){I.remove();
this._adjustChildListElementHeight();
}},_requestItems:function(J){var I=this,K=(this.get_maxResultCount()==-1);
clearTimeout(this._requestInProgress);
this._requestInProgress=setTimeout(function(){I.query(J,K);
},this._requestDelay);
},_onError:function(I){var J=c.RadSearchBox._extractErrorMessage(I);
alert(J);
},_query:function(I){if(this.get_isUsingODataSource()){this._doODataSourceCall(I);
}else{if(this.get_webServiceSettings()){this._doWebServiceCall(I);
}else{this._doCallback(I);
}}},_sanitizeODataResponse:function(K){var I=[],J=null;
a.each(K,function(L,M){J={};
a.extend(true,J,M.Attributes);
delete J.__metadata;
I[L]={Text:M.Text,Value:M.Value,DataItem:J};
});
return I;
},_normalizeDataItem:function(K){var I={},M=K.DataItem;
for(var J=0,L=M.length;
J<L;
J++){I[M[J].Key]=M[J].Value;
}K.DataItem=I;
},_renderResult:function(K,O){var L=null,J=null,I=k,N,Q=[];
if(this.get_clientTemplate()){I+=" "+m;
}for(var M=0,P=K.length;
M<P;
M++){if(O){this._normalizeDataItem(K[M]);
}J=K[M];
L={text:J.Text,value:J.Value,dataItem:J.DataItem};
if(this.get_clientTemplate()){N=c.TemplateRenderer.renderTemplate(J,this,this);
}else{N=J.Text;
}Q[M]=this._createChildListItemElement(N,I,L);
}return Q;
},_applyEmptyMessage:function(){var I=a(this.get_inputElement());
I.val(this.get_emptyMessage());
I.addClass(C);
},_removeEmptyMessage:function(){var I=a(this.get_inputElement());
if(I.hasClass(C)){I.val("");
I.removeClass(C);
}},_updateInputValue:function(I){if(I&&I.text){this.get_inputElement().value=I.text;
}},_adjustInitialInnerWrapWidth:function(){if(a(this._element).find(i+v).length==0){return;
}var M=this,I=a(M._element),J=I.find(i+t),L=J.outerWidth()-J.width(),K=I.find(i+v);
J.width(I.width()-(K.outerWidth()+L));
},_adjustInputWidth:function(){var Q=this,I=a(Q._element),O=Q.get_inputElement(),J=a(O),P=(J.outerWidth(G)-J.width()),N=I.find(i+t).width(),M=I.find(i+e).outerWidth(G)||0,L=I.find(i+g).outerWidth(G)||0,K=I.find(i+h).outerWidth(G)||0,R=I.find(i+y).outerWidth(G)||0;
var S=N-(M+L+K+R+P);
if(S>0){O.style.width=S+"px";
}},_adjustPopupWidth:function(L){var J=this._getPopupjQueryElement(),I=a(this._element),M=L||0,K=I.outerWidth()-M;
J.outerWidth(K);
},_serializePostbackCommand:function(I){var J=b.serialize(I);
return encodeURIComponent(J.replace(/'/g,"&squote"));
},_getSelectedContextItem:function(){var I=this.get_searchContext();
if(I){return I.get_selectedItem();
}return null;
},_getInnerWrapElement:function(){if(!this._innerWrapElement){this._innerWrapElement=a(this._element).find(i+t).get(0);
}return this._innerWrapElement;
},_getPopupjQueryElement:function(){if(!this._popupElement){this._popupElement=a(this.get_dropDownElement()).find(i+n);
}return this._popupElement;
},_doODataSourceCall:function(I){var M=this,Q=this.get_odataClientSettings().DataTextField,K=this.get_dataContextKeyField(),P=this._getSelectedContextItem(),L=P?P.get_key():"",O=I.showAllResults?"":this.get_maxResultCount(),J={ShowAllResults:I.showAllResults};
var N={events:{requesting:function(R){R.set_filterExpressions({field:Q,operator:q[M.get_filter()],value:I.searchString});
if(K&&L){R.set_filterExpressions({field:K,operator:q[2],value:L});
}var S=R.get_options();
S.context=J;
S.data.take=O;
},success:function(R){if(!M._disposed){M._onODataResponse(R,arguments[2]);
}},fail:function(R){if(!M._disposed){M._onODataError(R);
}}}};
this._flatBinder.fetch(N);
},_onODataResponse:function(M,I){var J,K,L=I?!I.ShowAllResults:true;
J=this._sanitizeODataResponse(M);
K=this._renderResult(J);
this._populateDropDown(a(K),L);
},_onODataError:function(I){alert("Request failed");
},_doWebServiceCall:function(I){var O=this._webServiceLoader,L=this.get_webServiceSettings().get_isWcf()?O._serializeDictionaryAsKeyValuePairs(I.userContext):I.userContext,M=this._getSelectedContextItem(),N={Text:I.searchString,ShowAllResults:I.showAllResults,SelectedContextItem:!M?null:{Text:M.get_text(),Key:M.get_key(),ImageUrl:M.get_imageUrl()},UserContext:L},J={ShowAllResults:I.showAllResults},K={context:N};
O.loadData(K,J);
},_onWebServiceResponse:function(N,I){if(this._disposed){return;
}var J=I.get_data(),K=this.get_webServiceSettings().get_isWcf(),L=this._renderResult(J,K),M=!I.get_context().ShowAllResults;
this._populateDropDown(a(L),M);
},_onWebServiceError:function(J,I){if(this._disposed){return;
}this._onError(I);
},_doCallback:function(L){var O=this._getSelectedContextItem(),I={Text:L.searchString,ShowAllResults:L.showAllResults,SelectedContextItem:!O?null:{Text:O.get_text(),Key:O.get_key(),ImageUrl:O.get_imageUrl()},UserContext:L.userContext},N=x(this._onCallbackResponse,this),M=x(this._onCallbackError,this),K=new c.CallbackSettings({id:this._uniqueId,arguments:b.serialize(I),context:{ShowAllResults:L.showAllResults},onCallbackSuccess:N,onCallbackError:M,isAsync:true}),J=new c.CallbackLoader(K);
J.invokeCallbackMethod();
},_onCallbackResponse:function(M,J){if(this._disposed){return;
}var N=M.split("_$$_"),K=a.parseJSON(N[0]),I=a(N[1]),L=J?!J.ShowAllResults:true;
I.each(function(P,O){O._data=K[P];
});
this._populateDropDown(I,L);
},_onCallbackError:function(I){if(this._disposed){return;
}this._onError(I);
},query:function(J,K){if(!this.get_enableAutoComplete()||!this.get_enabled()){return;
}var I={searchString:J,showAllResults:K,userContext:{}};
if(!J||J.length<this.get_minFilterLength()){this.clear();
}else{if(!this._raiseDataRequesting(I)){this._query(I);
}}},clear:function(){var I=this.get_childListElement();
if(I){I.innerHTML="";
...
},get_text:function(){var I=a(this.get_inputElement());
return I.hasClass(C)?"":I.val();
},get_buttons:function(){this._ensureChildControls();
return this._buttons;
},get_dropDownElement:function(){if(!this._dropDownElement){this._dropDownElement=a(this._element).find(i+o).get(0);
}return this._dropDownElement;
},get_inputElement:function(){if(!this._inputElement){this._inputElement=a(this._element).find(i+u).get(0);
...
this._webServiceSettings=new c.WebServiceSettings(I);
},get_isUsingODataSource:function(){return this._odataClientSettings!=null;
},get_flatModel:function(){var I=this.get_odataClientSettings();
return c.RadODataDataSource.Binder.Flat.Model(I);
},get_clientTemplate:function(){return this._clientTemplate;
},set_clientTemplate:function(I){this._clientTemplate=I;
},set_enableAutoComplete:function(I){if(I){this._initializeAutoComplete();
}else{this._disposeAutoComplete();
}this._enableAutoComplete=I;
},set_emptyMessage:function(I){this._emptyMessage=I;
this._applyEmptyMessage();
},get_emptyMessage:function(){return this._emptyMessage;
},get_searchContext:function(){return this._searchContext;
},get_enabled:function(){return this._enabled;
},set_enabled:function(J){if(this.get_enabled()===J){return;
}if(!J){c.RadSearchBox._toggleFocusedState.call(this,J,D);
}this._enabled=J;
a(this._getInnerWrapElement()).toggleClass(B,!J);
c.RadSearchBox._toggleAttribute(this.get_inputElement(),"disabled",!J,true);
var I=this.get_searchContext();
if(I){I.set_enabled(J);
}},trackChanges:function(){this._enableClientStatePersistence=true;
},commitChanges:function(){this._logEntriesJson=this._log.serialize();
this.updateClientState();
this._enableClientStatePersistence=false;
},saveClientState:function(){var I=this._getSelectedContextItem(),J=I?I.get_index():-1;
return'{"enabled":'+this._enabled+',"selectedContextIndex":'+J+',"logEntries":'+this._logEntriesJson+"}";
},_onButtonClick:function(J){var I=J.currentTarget._button;
this._raiseButtonCommand(I);
},_onSearchButtonClick:function(I){this._raiseSearch();
},_onShowAllClick:function(I){this.query(this.get_text(),true);
},_onKeyUp:function(I){var K=I.keyCode,J=Sys.UI.Key;
if(K!=J.up&&K!=J.down&&K!=J.enter&&K!=J.esc){this._requestItems(this.get_text());
}},_onPaste:function(I){this._requestItems(this.get_text());
},_onKeyDown:function(I){var M=I.keyCode,K=Sys.UI.Key,L=this._keyboardNavigator;
switch(M){case K.up:I.preventDefault();
if(this.get_enableAutoComplete()){L._updateHighlightedItem(c.HighlightItemDirection.Up);
}break;
case K.down:I.preventDefault();
if(this.get_enableAutoComplete()){L._updateHighlightedItem(c.HighlightItemDirection.Down);
}break;
case K.enter:var J=null;
I.preventDefault();
if(this._highlightedItem){J=this._highlightedItem._data;
this._updateInputValue(J);
}this._raiseSearch(J);
this.clear();
break;
case K.esc:I.preventDefault();
this.get_inputElement().value="";
this.clear();
break;
}},_onFocus:function(I){this._removeEmptyMessage();
c.RadSearchBox._toggleFocusedState.call(this,true,D);
},_onBlur:function(I){if(this.get_text()==""){this._applyEmptyMessage();
}c.RadSearchBox._toggleFocusedState.call(this,false,D);
this.clear();
},_onDocumentMouseDown:function(I){this.clear();
},_onItemMouseOver:function(I){var J=c.RadSearchBox._getTarget(I);
this._keyboardNavigator._highlightItem(J);
},_onItemMouseOut:function(I){this._keyboardNavigator._clearHighlightedItem();
},_onItemClick:function(I){var J=c.RadSearchBox._getTarget(I);
this._updateInputValue(J._data);
this._raiseSearch(J._data);
this.clear();
},_onClientLoad:function(){this._raiseClientEvent("load",{});
},_onSearch:function(I){this._raiseClientEvent("search",I);
},_onButtonCommand:function(I){this._raiseClientEvent("buttonCommand",I);
},_onDataRequesting:function(I){var J=a.extendEventArgs(new Sys.CancelEventArgs(),I);
return a.raiseCancellableControlEvent(this,"dataRequesting",J);
},_raiseClientEvent:function(K,I){var J=a.extendEventArgs(new Sys.EventArgs(),I);
a.raiseControlEvent(this,K,J);
},_raiseSearch:function(J){var I={text:J?J.text:this.get_text(),value:J?J.value:null,dataItem:J?J.dataItem:null};
this._onSearch(I);
if(this._postBackOnSearch){I.type=c.SearchBoxCommand.Search;
this._postback(I);
}},_raiseButtonCommand:function(I){var J={commandName:I.get_commandName(),commandArgument:I.get_commandArgument()};
this._onButtonCommand(J);
if(this._postBackOnButtonCommand){J.type=c.SearchBoxCommand.ButtonCommand;
this._postback(J);
}},_raiseDataRequesting:function(J){var I={text:J.searchString,showAllResults:J.showAllResults,userContext:J.userContext};
return this._onDataRequesting(I);
},_postback:function(I){var J=this._serializePostbackCommand(I);
eval(this._postBackReference.replace("arguments",J));
}};
a.registerControlProperties(c.RadSearchBox,{buttonData:null,filter:0,maxResultCount:-1,minFilterLength:1,dataContextKeyField:"",odataClientSettings:null,searchContextData:null});
...
(function() {
    function loadHandler() {
        ...
        hf.value += ';;System.Web.Extensions, Version=4.0.0.0, Culture=neutral, PublicKeyToken=31bf3856ad364e35:en-US:2caafa8b-6f58-4a66-a3cd-4d77ee6a1707:ea597d4b:b25378d2;Telerik.Web.UI:en-US:ac331549-681c-4402-9fb7-09ec3c579ee8:16e4e7cd:f7645509:22a6274a:24ee1bba:88144a7a:1e771326:eaae47ab';                                    
